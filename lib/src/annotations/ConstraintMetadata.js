...
var ConstraintMetadata = /** @class */ (function () {
    function ConstraintMetadata(ctx, next) {
        this.ctx = ctx;
        this.next = next;
    }
    ConstraintMetadata.prototype.setData = function (type, prototype, meta) {
        var _a;
        if (type === metadataType_1.default.Constraint) {
            if (!this.ctx.model.has(prototype)) {
                this.ctx.model.set(prototype, {});
            }
            var metadata = this.ctx.model.get(prototype);
            if (!metadata.constraints) {
                metadata.constraints = (_a = {}, _a[meta.propertyName] = constraintOption_1.default.NONE, _a);
            }
            if (!metadata.constraints[meta.propertyName]) {
                metadata.constraints[meta.propertyName] = constraintOption_1.default.NONE;
            }
    ...
    ConstraintMetadata.prototype.getData = function (type, prototype) {
        var _a;
        var args = [];
