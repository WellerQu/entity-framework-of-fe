...
var EntitySet = /** @class */ (function () {
    ...
    EntitySet.prototype.parseOriginDataToEntity = function (originData) {
        // 无数据
        if (isEmpty_1.default(originData)) {
            return null;
        }
        var entity = this.fill(originData);
        return entity;
    };
    ...
    EntitySet.prototype.cleanSet = function () {
        var _this = this;
        Array.from(this.set).forEach(function (item) {
            item.offPropertyBeforeChange(_this.onPropertyBeforeChange);
            item.offPropertyAfterChange(_this.onPropertyAfterChange);
        });
        this.set.clear();
        return this;
    };
    ...
    EntitySet.prototype.add = function () {
        ...
        var addedItems = entities.filter(function (item) { return !!item; });
    ...
    EntitySet.prototype.remove = function () {
        ...
        entities.filter(function (item) { return !!item; }).forEach(function (removedItem) {
            var tracer = Array.from(_this.set)
                .find(function (item) { return (item.rawObject === removedItem || item.proxyObject === removedItem) &&
            ...
            if (tracer) {
                ...
                tracer.offPropertyBeforeChange(_this.onPropertyBeforeChange);
                tracer.offPropertyAfterChange(_this.onPropertyAfterChange);
                tracer.revoke();
            }
        });
        return this;
    ...
    EntitySet.prototype.attach = function () {
        ...
        var attachedItems = entities.filter(function (item) { return !!item; });
        for (var i = 0; i < attachedItems.length; i++) {
            var attachedItem = attachedItems[i];
    ...
    EntitySet.prototype.detach = function () {
        ...
        var detachedItems = entities.filter(function (item) { return !!item; });
        var _loop_1 = function (i) {
            var detachedItem = detachedItems[i];
            var stateTrace = Array.from(this_1.set)
                .find(function (item) {
                return (item.proxyObject === detachedItem || item.rawObject === detachedItem) &&
                    item.state !== entityState_1.default.Detached &&
                    item.state !== entityState_1.default.Deleted;
            });
            if (stateTrace) {
                this_1.wMap.delete(detachedItem);
                stateTrace.state = entityState_1.default.Detached;
                stateTrace.offPropertyBeforeChange(this_1.onPropertyBeforeChange);
                stateTrace.offPropertyAfterChange(this_1.onPropertyAfterChange);
                stateTrace.revoke();
            }
        };
        var this_1 = this;
    ...
    EntitySet.prototype.find = function () {
        ...
        var stateTrace = Array.from(this.set).find(function (item) {
            var keys = entityMetadataManager_1.default.getPrimaryKeys(item.proxyObject.constructor.prototype);
            if (keys.length === 0) {
                return false;
            }
    ...
    EntitySet.prototype.filter = function (fn) {
        ...
        return stateTraces.map(function (item) { return item.proxyObject; });
    };
    EntitySet.prototype.toList = function () {
        return Array.from(this.set).map(function (item) { return Object.freeze(item.proxyObject); });
    };
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_c) {
                ...
                _a = queryMeta.mapParameters, mapParameters = _a === void 0 ? function () {
                    ...
                    return a;
                } : _a, _b = queryMeta.mapEntity, mapEntity = _b === void 0 ? function (p) { return p; } : _b;
                ...
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        _this.ctx.configuration
                            ...
                            .then(function (res) { return res.json(); }, reject)
                            .then(function (json) {
                            var data = mapEntity(json);
                            var first = Array.isArray(data) ? data[0] : data;
                            var entity = _this.parseOriginDataToEntity(first);
                            return Promise.all(requests.map(function (fn) { return fn(entity); })).then(function () {
                                entity && _this.attach(entity);
                                resolve(json);
                            }, reject);
                        }, reject);
                    })];
            ...
            return __generator(this, function (_c) {
                ...
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        _this.ctx.configuration
                            ...
                            .then(function (json) {
                            var data = mapEntity(json);
                            var parse = _this.parseOriginDataToEntity.bind(_this);
                            var entities = (data || []).map(parse).filter(function (item) { return !isEmpty_1.default(item); });
                            ...
                            return Promise.all(promises)
                                .then(function () {
                                _this.attach.apply(_this, __spread(entities));
                                resolve(json);
                            }, reject);
                        }, reject);
                    })];
     ...
     */
    EntitySet.prototype.entry = function (originData, entity) {
        var target = Array.from(this.set).find(function (item) { return item.rawObject === entity || item.proxyObject === entity; });
        if (!target) {
            throw new Error('实例不在上下文中');
        }
        var instance = entityMetadataManager_1.default.entry(originData, this.entityMetadata.type, false);
        var keys = Object.keys(instance);
        var rawObject = target.rawObject;
        keys.forEach(function (key) {
            var data = Reflect.get(instance, key);
     ...
     * @returns 填充数据的实例
     */
    EntitySet.prototype.fill = function (originData, isomorphism) {
        if (isomorphism === void 0) { isomorphism = false; }
        var instance = entityMetadataManager_1.default.entry(originData, this.entityMetadata.type, isomorphism);
        var keys = Object.keys(instance);
        keys.forEach(function (key) {
            var data = Reflect.get(instance, key);
     ...
     * @returns 原始数据
     */
    EntitySet.prototype.reverse = function (entity) {
        return entityMetadataManager_1.default.reverse(entity, this.entityMetadata.type);
    };
    EntitySet.prototype.rawQuery = function (query) {
        var _this = this;
        var requests = Object.values(this.ownNavigatorRequests);
        var fetchData = this.ctx.configuration.fetchData;
            ...
            query(fetchData)
                ...
                if (Array.isArray(originData)) {
                    for (var i = 0; i < originData.length; i++) {
                        var newEntity = _this.parseOriginDataToEntity(originData[i]);
                        if (newEntity) {
                    ...
                    var newEntity = _this.parseOriginDataToEntity(originData);
                    if (newEntity) {
                        entities.push(newEntity);
                    }
                }
    ...
    EntitySet.prototype.applyConstraints = function (special, rawObject) {
        var members = entityMetadataManager_1.default.getMembers(this.entityMetadata.type.prototype);
        var constraints = entityMetadataManager_1.default.getMemberConstraints(this.entityMetadata.type.prototype);
        return members.reduce(function (params, m) {
            var value = Reflect.get(rawObject, m.propertyName);
            var allConstraints = constraints[m.propertyName];
            if (!allConstraints) {
                Reflect.set(params, m.fieldName, value);
                return params;
            }
            if ((allConstraints & constraints_1.default.READ_ONLY) === constraints_1.default.READ_ONLY) {
                return params;
            }
            if ((allConstraints & special) === special && !isEmpty_1.default(value)) {
                Reflect.set(params, m.fieldName, value);
                return params;
            }
            return params;
    ...
    EntitySet.prototype.synchronizeAddedState = function (item) {
        return __awaiter(this, void 0, void 0, function () {
            var rawObject, behavior, _a, mapParameters, _b, mapEntity, applyConstraintsParams, params;
            var _this = this;
            return __generator(this, function (_c) {
                rawObject = item.rawObject;
                behavior = entityMetadataManager_1.default.getBehavior(rawObject.constructor.prototype, 'add');
                if (!behavior) {
                    return [2 /*return*/, (Promise.reject(new Error(rawObject.constructor.name + " \u6CA1\u6709\u914D\u7F6EAdd behavior")))];
                }
                _a = behavior.mapParameters, mapParameters = _a === void 0 ? identity : _a, _b = behavior.mapEntity, mapEntity = _b === void 0 ? identity : _b;
                applyConstraintsParams = this.applyConstraints(constraints_1.default.NON_EMPTY_ON_ADDED, rawObject);
                params = mapParameters(applyConstraintsParams);
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        _this.ctx.configuration.fetchData(behavior.url, { method: behavior.method }, params)
                            .then(function (res) { return res.json(); }, reject)
                            .then(function (json) {
    ...
    EntitySet.prototype.synchronizeDeletedState = function (item) {
        return __awaiter(this, void 0, void 0, function () {
            var rawObject, primaryKeys, behavior, _a, mapParameters, _b, mapEntity, params;
            var _this = this;
            return __generator(this, function (_c) {
                rawObject = item.rawObject;
                primaryKeys = entityMetadataManager_1.default.getPrimaryKeys(rawObject.constructor.prototype);
                behavior = entityMetadataManager_1.default.getBehavior(rawObject.constructor.prototype, 'delete');
                if (!behavior) {
                    return [2 /*return*/, (Promise.reject(new Error(rawObject.constructor.name + " \u6CA1\u6709\u914D\u7F6EDelete behavior")))];
                }
                _a = behavior.mapParameters, mapParameters = _a === void 0 ? identity : _a, _b = behavior.mapEntity, mapEntity = _b === void 0 ? identity : _b;
    ...
    EntitySet.prototype.synchronizeModifiedState = function (item) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_c) {
                rawObject = item.rawObject;
                behavior = entityMetadataManager_1.default.getBehavior(rawObject.constructor.prototype, 'update');
                if (!behavior) {
                    return [2 /*return*/, (Promise.reject(new Error(rawObject.constructor.name + " \u6CA1\u6709\u914D\u7F6E Update behavior")))];
                }
                _a = behavior.mapParameters, mapParameters = _a === void 0 ? identity : _a, _b = behavior.mapEntity, mapEntity = _b === void 0 ? identity : _b;
                applyConstraintsParams = this.applyConstraints(constraints_1.default.NON_EMPTY_ON_MODIFIED, rawObject);
                params = mapParameters(applyConstraintsParams);
                return [2 /*return*/, new Promise(function (resolve, reject) {
                        _this.ctx.configuration.fetchData(behavior.url, { method: behavior.method }, params)
                            .then(function (res) { return res.json(); }, reject)
    ...
    EntitySet.prototype.synchronizeState = function () {
        ...
        var _loop_2 = function (item) {
            ...
            if (state === entityState_1.default.Added) {
                promises.push(this_2.synchronizeAddedState(item));
                return "continue";
            }
            if (state === entityState_1.default.Deleted) {
                promises.push(this_2.synchronizeDeletedState(item));
                return "continue";
            }
            if (state === entityState_1.default.Modified) {
                promises.push(this_2.synchronizeModifiedState(item));
                return "continue";
            }
    ...
    EntitySet.prototype.onPropertyBeforeChange = function (tracer, e) {
        var constraints = entityMetadataManager_1.default.getMemberConstraint(this.entityMetadata.type.prototype, e.propertyName);
        if (!constraints) {
            return;
        }
        if ((constraints & constraints_1.default.READ_ONLY) === constraints_1.default.READ_ONLY) {
            throw new Error("\u65E0\u6CD5\u4FEE\u6539\u4E00\u4E2A\u6DFB\u52A0\u4E86READ_ONLY\u7EA6\u675F\u7684\u6210\u5458: " + e.propertyName);
        }
    };
    EntitySet.prototype.onPropertyAfterChange = function (tracer /*, e: PropertyChangeEvent<T, EntityState> */) {
